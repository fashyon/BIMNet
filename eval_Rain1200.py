...
from dataset_Rain1200 import TestDataset
from model import Bidirectional_Interactive_Multi_Scale_Network as BIMNet
...
class Session:
    def __init__(self):
        ...
        if torch.cuda.is_available():
            # self.net = BIMNet(dim=64).cuda() #BIMNet
            self.net = BIMNet(dim=144).cuda() #BIMNet_L
        if len(device_ids) > 1:
            # self.net = nn.DataParallel(BIMNet(dim=64)).cuda() #BIMNet
            self.net = nn.DataParallel(BIMNet(dim=144)).cuda() #BIMNet_L
